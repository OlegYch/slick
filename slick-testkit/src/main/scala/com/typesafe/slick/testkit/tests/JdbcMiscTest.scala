...
class JdbcMiscTest extends AsyncTest[JdbcTestDB] {
  ...
  def testStreamingCleanUpCalledOnMidIterationError: IO[Unit] = {
    ...
    val recordCleanup: Option[Throwable] => SlickAction[NoStream, Effect, Unit] =
  ...
  def testStreamingCleanUpFailurePropagatesOnSuccess: IO[Unit] = {
    ...
    val failingCleanup: Option[Throwable] => SlickAction[NoStream, Effect, Unit] =
  ...
  def testStreamingNestedCleanUpOuterRunsIfInnerFails: IO[Unit] = {
    ...
    val innerCleanup: Option[Throwable] => SlickAction[NoStream, Effect, Unit] =
      _ => DBIO.failed(innerException)

    val outerCleanup: Option[Throwable] => SlickAction[NoStream, Effect, Unit] =
      err => DBIO.successful(()).map { _ =>
  ...
  def testStreamingCleanUpCalledOnCancellation: IO[Unit] = {
    ...
    for {
      ...
      recordCleanup: (Option[Throwable] => SlickAction[NoStream, Effect, Unit]) =
        err => DBIO.successful(()).map { _ =>
          cleanupCalled.set(true)
          cleanupError.set(err)
