...
trait DBIOInstances3 {
  implicit val dbioInstance3: MonadError[[R] =>> DBIOAction[R, NoStream, Effect], Throwable] =
  ...
  implicit def dbioGroup3[A: Group]: Group[DBIOAction[A, NoStream, Effect]] =
    new DBIOGroup[A]

  implicit def dbioMonoid3[A: Monoid]: Monoid[DBIOAction[A, NoStream, Effect]] =
    new DBIOMonoid[A]

  implicit def dbioSemigroup3[A: Semigroup]: Semigroup[DBIOAction[A, NoStream, Effect]] =
    new DBIOSemigroup[A]
