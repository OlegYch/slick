...
trait BasicBackend { self =>
  ...
  protected[this] def logAction(a: SlickAction[NoStream, Nothing, ?]): Unit = {
    if (actionLogger.isDebugEnabled && a.isLogged) {
      ...
      val dump = new TreePrinter(prefix = "    ", firstPrefix = aPrefix, narrow = {
        case a: SlickAction[?, ?, ?] => a.nonFusedEquivalentAction
        case o                      => o
      }).get(logA)
  ...
  private[this] lazy val defaultActionLoggerAny: ActionListener[AnyK] =
    new ActionListener[AnyK] {
      override def around[R, H](a: SlickAction[_, _, R], exec: Any): Any = {
        logAction(a.asInstanceOf[SlickAction[NoStream, Nothing, ?]])
        exec
      }
    }
  ...
  trait BasicDatabaseDef[F[_]] extends AnyDatabaseDef { this: Database[F] =>
    ...
    /** Run a SlickAction and return the result in F[R]. */
    final def run[R](a: SlickAction[NoStream, Nothing, R]): F[R] =
    ...
    /** Open a streaming SlickAction as a Resource-backed Iterator.
      *
      * Finalizer order is:
      *   1) iterator.close()
      ...
      *   4) inflight-release
      */
    final def stream[T](a: SlickAction[Streaming[T], Nothing, ?]): Resource[F, Iterator[T]] =
    ...
    private def acquireStreamContextAndIterator[T](a: SlickAction[Streaming[T], Nothing, ?]): F[(Ref[F, ExecState], CloseableIterator[T], Option[Throwable] => F[Unit])] = {
      val F = asyncF
      F.uncancelable { _ =>
        val acquire =
    ...
    protected def interpretStream[T](
      ...
      val streamExec: F[(CloseableIterator[T], Option[Throwable] => F[Unit])] = a match {
        ...
        case sa: SynchronousDatabaseAction[?, ?, ?, ?] =>
          interpretStream(
            sa.nonFusedEquivalentAction.asInstanceOf[SlickAction[Streaming[T], Nothing, ?]],
            ctx
          )

        case AndThenAction(actions) =>
          val prefix = actions.init.asInstanceOf[IndexedSeq[SlickAction[NoStream, Nothing, Any]]]
          val last = actions.last.asInstanceOf[SlickAction[Streaming[T], Nothing, ?]]
        ...
        case FlatMapAction(base, f) =>
          interpret[Any](base.asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx).flatMap { v =>
            val next = f.asInstanceOf[Any => SlickAction[Streaming[T], Nothing, ?]](v)
            interpretStream(next, ctx)
          }

        case NamedAction(inner, _) =>
          interpretStream(inner.asInstanceOf[SlickAction[Streaming[T], Nothing, ?]], ctx)
        ...
        case CleanUpAction(base, f, keepFailure) =>
          ...
          interpretStream(base.asInstanceOf[SlickAction[Streaming[T], Nothing, ?]], ctx)
            .map {
              case (iter, baseCleanup) =>
                val combinedCleanup: Option[Throwable] => F[Unit] = err =>
                  ...
                  baseCleanup(err).attempt.flatMap { baseResult =>
                    ...
                    interpret[Any](f(errForOuter).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
                      .attempt
                      .flatMap {
            ...
            .recoverWith { case setupErr =>
              interpret[Any](f(Some(setupErr)).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
                .attempt
                .flatMap {
        ...
        case other =>
          ...
          // This preserves prior behaviour for library users whose SlickAction subclasses do
          // not match any of the known structural cases above.
          interpret[Seq[T]](other.asInstanceOf[SlickAction[NoStream, Nothing, Seq[T]]], ctx)
    ...
    /** The core recursive interpreter for `SlickAction` values.
      *
    ...
    protected def interpret[R](
                                a: SlickAction[NoStream, Nothing, R],
                                ctx: Ref[F, ExecState]
    ): F[R] = {
      val F = asyncF
      ...
      F.defer {
        val exec: F[R] = a match {
          ...
          case FlatMapAction(base, f) =>
            interpret[Any](base.asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
              .flatMap { v =>
                val next = f.asInstanceOf[Any => SlickAction[NoStream, Nothing, R]](v)
          ...
          case AndThenAction(actions) =>
            ...
            def run(pos: Int): F[Any] = {
              val fi = interpret[Any](actions(pos).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
              if (pos == last) fi
          ...
          case sa @ SequenceAction(actions) =>
            ...
            def run(pos: Int, acc: Vector[Any]): F[Vector[Any]] = {
              if (pos == len) F.pure(acc)
                ...
                interpret[Any](actions(pos).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
          ...
          case CleanUpAction(base, f, keepFailure) =>
            ...
            F.uncancelable { poll =>
              poll(interpret[R](base.asInstanceOf[SlickAction[NoStream, Nothing, R]], ctx))
                .guaranteeCase {
                  case Outcome.Succeeded(_) =>
                    interpret[Any](f(None).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx).void
                  case Outcome.Errored(err) =>
                    ...
                    interpret[Any](f(Some(err)).asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx).void
                  ...
                  case Outcome.Canceled() =>
                    ...
                    interpret[Any](
                      f(Some(new java.util.concurrent.CancellationException("DBIO action canceled")))
                        .asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx).attempt.void
                }
            }

          case FailedAction(inner) =>
            interpret[Any](inner.asInstanceOf[SlickAction[NoStream, Nothing, Any]], ctx)
              .attempt
              .flatMap {
          ...
          case NamedAction(inner, _) =>
            interpret[R](inner.asInstanceOf[SlickAction[NoStream, Nothing, R]], ctx)
          ...
          case TransactionalAction(inner, isolationLevel) =>
            ...
            asyncF.uncancelable { poll =>
              enterTransactionScope(ctx, isolationLevel) >>
              poll(interpret[R](inner.asInstanceOf[SlickAction[NoStream, Nothing, R]], ctx))
