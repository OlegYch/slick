...
trait DBIOInstances2 {
  type DBIOAll[R] = SlickAction[NoStream, Effect, R]
  implicit val dbioInstance2: MonadError[DBIOAll, Throwable] =
  ...
  implicit def dbioGroup2[A: Group]: Group[DBIOAll[A]] =
    new DBIOGroup[A]

  implicit def dbioMonoid2[A: Monoid]: Monoid[DBIOAll[A]] =
    new DBIOMonoid[A]

  implicit def dbioSemigroup2[A: Semigroup]: Semigroup[DBIOAll[A]] =
    new DBIOSemigroup[A]
