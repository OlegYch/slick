...
trait JdbcActionComponent extends SqlActionComponent { self: JdbcProfile =>
  ...
  class JdbcActionExtensionMethods[E <: Effect, R, S <: NoStream](a: SlickAction[S, E, R]) {
    ...
    /** Run this Action transactionally with the specified transaction isolation level.
      ...
      */
    def transactionally(ti: TransactionIsolation): SlickAction[S, E with Effect.Transactional, R] =
    ...
    def withTransactionIsolation(ti: TransactionIsolation): SlickAction[S, E, R] = {
      val isolated =
    ...
    def withStatementParameters(rsType: ResultSetType = null,
                                ...
                                fetchSize: Int = 0): SlickAction[S, E, R] =
